...
for j=1:19
    ...
    for i=1:5
        ...
        if j==19
            E(i,j)=-xv(i,j) + ((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j));
        elseif j==1
            E(i,j)=-xv(i,j) + 0.5*((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j)) + 0.5*xv(i,j+1)*sv(j)/sv(j+1);
        else
            E(i,j)=-xv(i,j) + eff*((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j)) + (1-eff)*xv(i,j+1)*sv(j)/sv(j+1);
        end
    end
end
end
