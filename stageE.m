...
for k=1:5
    ...
    for q=1:5
        for w=1:5
        amix=amix+(xv(q,j)/sv(j))*(xv(w,j)/sv(j))*(aij(q,w));
        end
    end
    bmix=0;
    for q=1:5
    ...
    for i=1:5
        ...
        for k=1:5
            big_sum=(xl(k,j)./sl(j))*A(k,i)/sum_xA(k);
        end
        ...
        z=P*v/(R*X(6,j));
        sum_ya= sum((xv(:,j)/sv(j))'.*aij(i,:));
        phi = 2.718^(bi(i)*((z)-1)/bmix-log((v-bmix)*P/(R*X(6,j)))+log(1+bmix/v)*(amix*bi(i)/bmix-2*(sum_ya))/(bmix*R*X(6,j)^1.5));
        if j==19
            E(i)=-xv(i,j) + ((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j));
        elseif j==1
            E(i)=-xv(i,j) + 0.5*((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j)) + 0.5*xv(i,j+1)*sv(j)/sv(j+1);
        else
            E(i)=-xv(i,j) + eff*((Psat/P)*(exp(yi)/phi))*xl(i,j)*sv(j)/(sl(j)) + (1-eff)*xv(i,j+1)*sv(j)/sv(j+1);
        end
    end
 end